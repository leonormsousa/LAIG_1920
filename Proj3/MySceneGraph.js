...
class MySceneGraph {
    ...
    parseAnimations(animationsNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            grandChildren = children[i].children;
            // Specifications for the current animations.
            ...
            for (var j = 0; j < grandChildren.length; j++) {
                ...
                for(let k=0; k<grandgrandChildren.length;k++){
                    nodeNamesGrand.push(grandgrandChildren[k].nodeName);
                }
                var translateID = nodeNamesGrand.indexOf("translate");
                var scaleID = nodeNamesGrand.indexOf("scale");
                var rotateID = nodeNamesGrand.indexOf("rotate");

                if (translateID!=0)
                    return "first element of keyframes in Animation with ID " + animationID + " must be translate";
                if (rotateID!=1)
                    return "second element of keyframes in Animation with ID " + animationID + " must be rotate";
                if (scaleID!=2)
                    return "third element of keyframes in Animation with ID " + animationID + " must be scale";

                var tx=this.reader.getFloat(grandgrandChildren[translateID], 'x');
                var ty=this.reader.getFloat(grandgrandChildren[translateID], 'y');
                var tz=this.reader.getFloat(grandgrandChildren[translateID], 'z');
                var rx=this.reader.getFloat(grandgrandChildren[rotateID], 'angle_x');
                var ry=this.reader.getFloat(grandgrandChildren[rotateID], 'angle_y');
                var rz=this.reader.getFloat(grandgrandChildren[rotateID], 'angle_z');
                var sx=this.reader.getFloat(grandgrandChildren[scaleID], 'x');
                var sy=this.reader.getFloat(grandgrandChildren[scaleID], 'y');
                var sz=this.reader.getFloat(grandgrandChildren[scaleID], 'z');

                var keyframe = new Keyframe(this.scene, keyframeInst, tx, ty, tz, rx, ry, rz, sx, sy, sz);
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (primitiveType == 'rectangle') {
               ...
               this.primitives[primitiveId] = plane;
                ...
                if (grandgrandChildren.length!=npointsV*npointsU)
                    return "number of control points " + grandgrandChildren.length + "different from npointsU*npointsV in primitive with ID = " + primitiveId;

                var points = new Array(npointsU);
                for (var o = 0; o < points.length; o++) {
                    points[o] = new Array(npointsV);
                }

                for (var o=0; o<npointsU; o++){
                    for (var w=0; w<npointsV; w++){
                        points[o][w]=[this.reader.getFloat(grandgrandChildren[o*npointsV+w], 'xx'), this.reader.getFloat(grandgrandChildren[o*npointsV+w], 'yy'), this.reader.getFloat(grandgrandChildren[o*npointsV+w], 'zz'), 1];
                    }
                }
                ...
                var cylinder = new Cylinder2(this.scene, primitiveId, base, top, height, slices, stacks);
